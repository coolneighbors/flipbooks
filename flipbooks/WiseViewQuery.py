...
class WiseViewQuery:
    ...
    def downloadData(self, url, i, ra, dec, outdir):
        fieldName = 'field-RA' + str(ra) + '-DEC' + str(dec) + '-' + str(i) + '.png'
        fname_dest = WiseViewQuery.downloadPNG(url, outdir, fieldName)
        return fname_dest

    def downloadPNGs(self, urls, ra, dec, outdir):
        processes = [self.pool.apply_async(WiseViewQuery.downloadData, args=(urls[i], i, ra, dec, outdir)) for i in range(len(urls))]
    ...
    def downloadWiseViewData(self, outdir, scale_factor=1.0, addGrid=False, gridCount=5, gridType = "Solid", gridColor = (0,0,0)):
        ...
        postProcessing.applyPNGModifications(flist, scale_factor, addGrid, gridCount, gridType, gridColor, self.pool)
